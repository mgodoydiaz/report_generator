...
**Promoción a producción 2026-08-03**: `main` en `876b852` (deploy Railway verificado: backend y frontend arriba). Del lote de scripts: `fix_specs_version_idel.py` **NO aplica en producción** — allí datos y specs ya son coherentes en `v1/v2/v3` (la guardia del script lo detectó y abortó; el formato `1/2/3` es solo de dev → dev no es espejo fiel de prod en IDEL). Los clones IDEL 2026 tampoco existen en prod (2026 v2/v3 son cohortes reales de 120/114 filas). **Pendientes de correr por Miguel en prod** (el clasificador bloquea writes a prod desde el agente): `fix_semaforo_color_overrides.py --org 1 --apply` (dry-run verificado: 10 specs, 0 ambiguos) y `fix_column_roles_idel.py --org 1 --apply` (dry-run verificado: agrega `logro_1`→Puntaje). Pendientes menores nuevos: 2 textos de display en DB aún dicen "v1, v2, v3" (description del spec 141 y nota del tab Tendencia); `derived_columns` de CV tiene un `slope` ordinal sin `ordinal_levels` (error tragado en silencio); decisión aparcada: filtro de tablas "Estudiantes en Riesgo" (propuesta de Miguel: bajo el primer cuartil); revisar en producción si IDEL 2026 v2/v3 son clones de 2025 como en dev.
...
> **Retiro de `Nombre_Norm` (2026-08-07, en `dev`)**: el nombre reordenado dejó de ser un dato persistido — es una función interna de lectura. Identidad: **RUT → `normalizar_nombre(Nombre)` calculado en memoria**; presentación siempre `Nombre` tal cual. Los 4 caminos de escritura ya no generan el par X/X_Norm (`SaveToMetric` además descarta columnas `X_Norm` entrantes, con warning); el kind `normalize_name` queda deprecado y su reemplazo es `entity_normalize` (agg/slope/delta) + kind `copy`. Los `entity_field` del DIA pasan a `["Año","Curso","Nombre"]` — agregar `Año` cierra la fuga de cohortes 2025→2026 (P1-1 del QA DIA 2026-08-03). Migración en dev: `scripts/quitar_normalize_name_pipelines.py` (pipelines vivos) y `scripts/retirar_nombre_norm.py` (desasocia la dimensión 22 de `metric_dimensions`; los valores históricos en `dimensions_json` quedan inertes y los lectores los ignoran). **Pendiente**: correr ambos scripts en producción con GO de Miguel.

| # | Pendiente | Notas |
|---|---|---|
| ~~P1~~ | ~~583 filas sin identidad en producción~~ **RESUELTO 2026-08-07** | Las 576 de DIA (LECTURA·DIAGNÓSTICO·2026) se repararon en dev y prod recuperando Nombre y Nombre_Norm desde los XLS fuente (join posicional por Curso + orden de N° de lista, validado con checksum de Logro 576/576; respaldos en scratchpad). Causa raíz: `SaveToMetric` descarta columnas sin match exacto + config obsoleta cacheada en `ACTIVE_RUNNERS` (run pausado 55 min el 2026-05-05). Además: prod no declaraba `Nombre_Norm` en `metric_dimensions` de la métrica 6 (drift vs dev, corregido con INSERT) — por eso la carga del 2026-08-06 dejó 217 filas sin normalizado (también corregidas). Verificación final: 18.200 pares Nombre/Nombre_Norm en prod, 0 inconsistentes bajo `normalizar_nombre`. Las 7 de SIMCE Panguipulli siguen sin nombre (vacías en el origen Aptus) pero tienen RUT. Scripts: `reparar_identidades_dia_2026.py`, `dia_ano_por_usuario.py`, `dia_establecimiento_canonico.py`. |
| P1 | **Métrica 7 (DIA por Pregunta) rota para 2025·DIAGNÓSTICO·LECTURA·Pullinque** | La carga del 2026-08-06 en prod insertó 132 filas donde deberían ser ~248 (15 de 31 preguntas en el curso auditado) y con Logro incorrecto (ej. preg 2: BD 19% vs Power BI 67%). El XLS de estudiantes de esa misma carga quedó perfecto (calza 1:1 con Power BI). Hipótesis: el extractor de PDF no parsea el formato 2025. Falta: conseguir los PDF de esa carga (no están en `Informes PHP` ni Descargas; probablemente en el volumen Railway `data/pipeline_runs/runs/21/`), borrar las 132 filas malas y recargar con el extractor corregido. Panguipulli 2025·DIAGNÓSTICO además tiene 0 preguntas (hueco previo). Nota: la métrica 6 mezcla granularidades (cargas 2025 antiguas = 1 fila por alumno×habilidad; nuevas = 1 fila por alumno). |
...
| P2 | Pipeline de SIMCE Panguipulli sin `description` en sus `file_specs` | Los tres recuadros de carga muestran el texto genérico en vez de decir qué archivo va. |
| P2 | Falta **probar una carga real en producción** del pipeline 22 `SIMCE Panguipulli (Aptus)` | Desplegado y verificado estructuralmente el 2026-08-05 (specs 200/201, métricas 17/18, 13 pasos), pero nunca corrió allá con archivos reales. Hacerlo con un mes **nuevo**: la carga suma, no reemplaza. Referencia dev: mayo 2025 = 443 filas en Estudiante + 45 en Habilidad. |
| P2 | Dimensión 22 `Nombre_Norm` **huérfana** en la métrica 24 | Tiene valores en `metric_data` pero no está en `metric_dimensions` para la métrica 24, así que el export la rotula `Dim_22` (fallback de `backend/routers/metrics.py`). Un export→import de ida y vuelta **pierde la columna en silencio**, porque el import matchea por nombre de dimensión vigente. Decidir: asociarla a la métrica o limpiar esos valores. Detectado 2026-08-03. |
| P2 | **2024 de EMN Aptus disponible pero sin cargar** | El histórico local tiene 2024 completo (II° medio, ABRIL/MAYO/SEPTIEMBRE): 1245 filas de estudiante, 425 de OA, 135 de habilidad. Cargarlo daría comparación año contra año, que el indicador 6 hoy no tiene. Ojo: aparecería `CIENCIAS NATURALES` como asignatura nueva (solo existe en 2024, no está en el mapeo de los specs) y `LogroNormalizado` viene mayormente nulo. Constructor listo en `scripts/construir_importables_emn.py`. |
...
| # | Pendiente | Notas |
|---|---|---|
| P1 | **2 placeholders `### COMPLETAR POR MIGUEL ###`** en el anexo de cargas | (a) qué hace el usuario cuando cambie el año, ya que DIA lo tiene fijo en 2026; (b) de qué menú de la plataforma DIA se descargan sus dos archivos. El tercero (cómo llegan los informes de Aptus) se resolvió el 2026-08-03: se documentó el recorrido `web.aptus.org → Recursos Pedagógicos → Ver Resultados → Tipo de proceso EMN → Proceso/Nivel/Asignatura → panel Informes`, con los 3 enlaces de Excel. |
