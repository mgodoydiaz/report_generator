...
- [~] **Pipeline DIA Matemáticas / Lectura** — portar `script_consolidar_DIA.py` artesanal del cliente a steps configurables. **Parcialmente cerrado en B6b (2026-05-04)**: ETL XLS cubierto (`metadata_cells` en `RunExcelETL` para B5/B6, kinds `row_mean_dynamic` + `row_threshold` + `normalize_name` en el engine, JSON pipelines en `data/pipelines/dia/`). Step `RunDIAPDFExtraction` registrado como stub que lanza `NotImplementedError` — pendiente de portar las funciones camelot+fitz literal del script artesanal. Ver `docs/desarrollo/script_dia_artesanal_referencia.md`.
