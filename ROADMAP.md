...
| v0.7.0 | 2026-07-30 | **Informes por período y motor único (fase 3)**. Selector "Generar informe" con última prueba / semestral / anual / personalizado (`periodos.py`), registro de informes Python en `reports/custom/`, asignatura obligatoria donde aplica, piloto SIMCE del motor único con 4 modos y riesgo persistente calibrado. Tipo de dimensión `date` con año derivado de la fecha. Filtros server-side en `/values`. Fixes de ETL: `Nombre`/`Nombre_Norm` en todos los caminos de escritura, `EnrichWithLookup` ya no pierde la llave homónima, guard de cobertura 0%. Branding neutro (pie = organización). Guía rápida de usuario con capturas + anexo de carga. Organización de prueba "Colegio Demo". 1566 tests verdes. Desplegado en producción con backfill de datos históricos. |
